...
WOE segun la agrupacion de edad:

```{r}
age_ds <- iv_ds$Tables$age_groups
age_ds$age_groups <- factor(age_ds$age_groups , levels=levels(dataset$age_groups))
ggplot(age_ds, aes(x=age_groups, y=WOE, fill = WOE))+
  scale_fill_gradient(low = "red", high = "green") +
  geom_bar(stat = "identity")
